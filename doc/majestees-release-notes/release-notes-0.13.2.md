Majesteescoin Core version 0.13.2 is now available from:

  <https://download.majestees.org/majestees-0.13.2.1/>
...
updates will be released anymore. Without security updates, using a majestees
...
ECDSA signatures inside Majesteescoin transactions now use validation using
...
Previous versions of Majesteescoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Majesteescoin Core was to raise the default minimum
relay fee.

Majesteescoin Core 0.13.2 will have a strict maximum size on the mempool. The
...
Majesteescoin Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running majesteesd without having to do any manual configuration.
...
Majesteescoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Majesteescoin Core will relay transactions with
...
entirely in Majesteescoin Core 0.13, and it is at this time undecided whether the
...
Majesteescoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Majesteescoin Core automatically creates a hidden service to listen on, without
manual configuration. Majesteescoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Majesteescoin Core is listening, and
...
Majesteescoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Majesteescoin
Core to use floating fees. By default, Majesteescoin Core will use floating
fees.
...
At all times, Majesteescoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) MTC.
Furthermore, Majesteescoin Core will never create transactions paying less than
...
`majestees.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
Then, to tunnel a SSL connection on 28998 to a RPC server bound on localhost on port 19334 do:

    stunnel -d 28998 -r 127.0.0.1:19334 -p stunnel.pem -P ''
...
caching. A sample config for apache2 could look like:
    ...
    <Location /majesteesrpc>
        ProxyPass http://127.0.0.1:8998/
        ProxyPassReverse http://127.0.0.1:8998/
        # optional enable digest auth
        # AuthType Digest
        # ...

        # optional bypass majesteesd rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< majesteesrpc:<password>
    </Location>
...
Restarting majesteesd will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in majestees.conf
...
majestees-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/majestees-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Majesteescoin Core code base has
...
- `majestees-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `majestees-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the Majesteescoin scripting system that in combination with
...
Majesteescoin transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Majesteescoin Core.
...
The two phases are distinct in the Majesteescoin-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Majesteescoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Majesteescoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Majesteescoin block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of majesteess they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of majesteess being spent is exactly
...
- **Increased security for multisig:** Majesteescoin addresses (both P2PKH addresses
  ...
  believed to be provided by Majesteescoin's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Majesteescoin's current ledger (called the UTXO set),
  ...
  note that Majesteescoin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Majesteescoin users to individually opt-in to almost any change in the Majesteescoin
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Majesteescoin Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Majesteescoin
client to Majesteescoin Core 0.13.2 to signal for segwit support. When a
...
Since Majesteescoin Core 0.10.0, nodes have defaulted to only relaying and
...
- `majestees-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `majestees-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
